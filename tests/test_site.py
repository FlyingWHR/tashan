...
def static_checks():
    ...
    # 3. board is trust-ranked — every row carries a trust score
    untrusted = [r["slug"] for r in board if r.get("trust") is None]
    check("every board row has a trust score", not untrusted,
          f"{len(untrusted)} rows without trust")
