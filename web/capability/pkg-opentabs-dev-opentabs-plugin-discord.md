# Opentabs Plugin Discord

> OpenTabs plugin for Discord

## Facts
- Page: https://tashan.sh/capability/pkg-opentabs-dev-opentabs-plugin-discord
- tashan id: pkg:@opentabs-dev/opentabs-plugin-discord
- Source: https://github.com/opentabs-dev/opentabs
- npm: https://www.npmjs.com/package/@opentabs-dev/opentabs-plugin-discord
- Type: npm
- Category: other
- tashan score: 58.0 / 100
- Adoption: 31.0
- Upkeep: 68.0
- Freshness: 97.0
- Evidence coverage: 100% of the inputs this score can use
- Health: active
- Instruction depth: not yet graded
- npm downloads: 176/week
- Official: no

## Install

```sh
claude mcp add opentabs-dev-opentabs-plugin-discord -- npx -y @opentabs-dev/opentabs-plugin-discord
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-03 by tashan (https://tashan.sh) from public evidence. Scorer s5.
