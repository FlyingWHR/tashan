# Atlassian API Token

> Bundles the Atlassian Remote MCP Server (Rovo) so Claude Code can access Jira, Confluence, Jira Service Management, Bitbucket, and Compass using a personal API token (Basic auth) instead of interactive SSO login.

## Facts
- Page: https://tashan.sh/capability/plugin-duylam-claude-code-engineering-assembly-atlassian-api-token
- tashan id: plugin:duylam/claude-code-engineering-assembly/atlassian-api-token
- Source: https://github.com/duylam/claude-code-engineering-assembly
- Type: plugin
- Category: productivity
- tashan score: 14.0 / 100
- Adoption: 7.0
- Upkeep: 33.0
- Freshness: not measured
- Evidence coverage: 62% of the inputs this score can use
- Health: not measured
- Instruction depth: not yet graded
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add duylam/claude-code-engineering-assembly
/plugin install atlassian-api-token@engineering-assembly
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-04 by tashan (https://tashan.sh) from public evidence. Scorer s5.
