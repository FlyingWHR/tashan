# Doc

> Multi-purpose documentation toolkit (generation, explanation, diagrams, health)

## Facts
- Page: https://tashan.sh/capability/plugin-joaquimscosta-arkhe-claude-plugins-doc
- tashan id: plugin:joaquimscosta/arkhe-claude-plugins/doc
- Source: https://github.com/joaquimscosta/arkhe-claude-plugins
- Type: plugin
- Category: docs
- tashan score: 29.0 / 100
- Adoption: 7.0
- Upkeep: 64.0
- Freshness: 62.0
- Evidence coverage: 84% of the inputs this score can use
- Health: active
- Instruction depth: not yet graded
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add joaquimscosta/arkhe-claude-plugins
/plugin install doc@arkhe-claude-plugins
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-03 by tashan (https://tashan.sh) from public evidence. Scorer s5.
