# Urdb

> Search URDB for product integrity scores, enshittification events, and change tracking.

## Facts
- Page: https://tashan.sh/capability/pkg-urdb-mcp
- tashan id: pkg:urdb-mcp
- Source: https://github.com/GetMystAdmin/urdb-mcp
- npm: https://www.npmjs.com/package/urdb-mcp
- Type: npm
- Category: data
- tashan score: 31.0 / 100
- Adoption: 22.0
- Upkeep: 42.0
- Freshness: 59.0
- Evidence coverage: 100% of the inputs this score can use
- Health: active
- Instruction depth: not yet graded
- npm downloads: 39/week
- Official: no

## Install

```sh
claude mcp add urdb -- npx -y urdb-mcp
```

## Security audit
- Known advisories: 0
- Install-time script: none declared
- Build provenance: not attested

Permissions are read from DECLARED dependencies only. Nothing is executed, so an empty result means "nothing declared", never "nothing possible".

---
Measured 2026-08-04 by tashan (https://tashan.sh) from public evidence. Scorer s5.
