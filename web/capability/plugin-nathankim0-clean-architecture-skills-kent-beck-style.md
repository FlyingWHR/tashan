# Kent Beck Style

> 켄트 벡의 리팩토링 철학과 단순 설계 원칙에 따라 코드를 리뷰하고 가이드를 제공합니다. 코드 냄새 감지, 리팩토링 기법, 점진적 개선을 다룹니다.

## Facts
- Page: https://tashan.sh/capability/plugin-nathankim0-clean-architecture-skills-kent-beck-style
- tashan id: plugin:nathankim0/clean-architecture-skills/kent-beck-style
- Source: https://github.com/nathankim0/clean-architecture-skills
- Type: plugin
- Category: search
- tashan score: 21.0 / 100
- Adoption: 7.0
- Upkeep: 41.0
- Freshness: 50.0
- Evidence coverage: 84% of the inputs this score can use
- Health: abandoned
- Instruction depth: not yet graded
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add nathankim0/clean-architecture-skills
/plugin install kent-beck-style@clean-architecture-skills
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-04 by tashan (https://tashan.sh) from public evidence. Scorer s5.
