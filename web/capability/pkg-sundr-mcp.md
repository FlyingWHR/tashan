# Sundr Repair Advisor

> Repair or replace? Get device repair costs, local shops, trade-in values.

## Facts
- Page: https://tashan.sh/capability/pkg-sundr-mcp
- tashan id: pkg:sundr-mcp
- Source: https://github.com/KeithWyatt/sundr
- npm: https://www.npmjs.com/package/sundr-mcp
- Type: npm
- Category: finance
- tashan score: 31.0 / 100
- Adoption: 22.0
- Upkeep: 46.0
- Freshness: 55.0
- Evidence coverage: 100% of the inputs this score can use
- Health: active
- Instruction depth: not yet graded
- npm downloads: 37/week
- Official: no

## Install

```sh
claude mcp add sundr -- npx -y sundr-mcp
```

## Security audit
- Known advisories: 0
- Install-time script: none declared
- Build provenance: not attested

Permissions are read from DECLARED dependencies only. Nothing is executed, so an empty result means "nothing declared", never "nothing possible".

---
Measured 2026-08-03 by tashan (https://tashan.sh) from public evidence. Scorer s5.
