# Wjs Mining Articles

> Mine a video's SRT — a monologue / 讲解, or an interview where 王建硕 is one speaker — into multiple standalone 微信公众号 articles, one per distinct topic.

## Facts
- Page: https://tashan.sh/capability/plugin-jianshuo-claude-skills-wjs-mining-articles
- tashan id: plugin:jianshuo/claude-skills/wjs-mining-articles
- Source: https://github.com/jianshuo/claude-skills
- Type: plugin
- Category: productivity
- tashan score: 28.0 / 100
- Adoption: 7.0
- Upkeep: 67.0
- Freshness: not measured
- Evidence coverage: 62% of the inputs this score can use
- Health: not measured
- Instruction depth: not yet graded
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add jianshuo/claude-skills
/plugin install wjs-mining-articles@claude-skills
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-03 by tashan (https://tashan.sh) from public evidence. Scorer s5.
