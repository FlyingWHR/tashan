# Loop And Gate Build Kit

> A judgment layer for building software with an agentic loop — eleven human-decision gates plus the loop-and-gate operating skill.

## Facts
- Page: https://tashan.sh/capability/plugin-slogsdon-loop-and-gate-build-kit-loop-and-gate-build-kit
- tashan id: plugin:slogsdon/loop-and-gate-build-kit/loop-and-gate-build-kit
- Source: https://github.com/slogsdon/loop-and-gate-build-kit
- Type: plugin
- Category: productivity
- tashan score: 36.0 / 100
- Adoption: 7.0
- Upkeep: 63.0
- Freshness: 97.0
- Evidence coverage: 84% of the inputs this score can use
- Health: active
- Instruction depth: not yet graded
- GitHub stars: 0
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add anthropics/claude-plugins-community
/plugin install loop-and-gate-build-kit@claude-community
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-04 by tashan (https://tashan.sh) from public evidence. Scorer s5.
