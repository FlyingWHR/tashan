# Filesystem · ai-capabilities-suite

> Advanced filesystem operations with strict security boundaries for AI agents

## Facts
- Page: https://tashan.sh/capability/pkg-ai-capabilities-suite-mcp-filesystem
- tashan id: pkg:@ai-capabilities-suite/mcp-filesystem
- Source: https://github.com/Digital-Defiance/ai-capabilities-suite
- npm: https://www.npmjs.com/package/@ai-capabilities-suite/mcp-filesystem
- Type: npm
- Category: files
- tashan score: 29.0 / 100
- Adoption: 27.0
- Upkeep: 45.0
- Freshness: 42.0
- Evidence coverage: 100% of the inputs this score can use
- Health: abandoned
- Instruction depth: not yet graded
- npm downloads: 84/week
- License: MIT
- Official: no

## Install

```sh
claude mcp add filesystem -- npx -y @ai-capabilities-suite/mcp-filesystem
```

## Security audit
- Known advisories: 0
- Install-time script: none declared
- Build provenance: not attested
- Declared permission surface: filesystem

Permissions are read from DECLARED dependencies only. Nothing is executed, so an empty result means "nothing declared", never "nothing possible".

---
Measured 2026-08-05 by tashan (https://tashan.sh) from public evidence. Scorer s5.
