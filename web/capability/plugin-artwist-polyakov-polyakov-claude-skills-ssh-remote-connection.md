# SSH Remote Connection

> SSH подключение к удалённым серверам с agent forwarding

## Facts
- Page: https://tashan.sh/capability/plugin-artwist-polyakov-polyakov-claude-skills-ssh-remote-connection
- tashan id: plugin:artwist-polyakov/polyakov-claude-skills/ssh-remote-connection
- Source: https://github.com/artwist-polyakov/polyakov-claude-skills
- Type: plugin
- Category: devtools
- tashan score: 29.0 / 100
- Adoption: 7.0
- Upkeep: 77.0
- Freshness: 51.0
- Evidence coverage: 84% of the inputs this score can use
- Health: abandoned
- Instruction depth: not yet graded
- License: MIT
- Official: no

## Install

```sh
/plugin marketplace add artwist-polyakov/polyakov-claude-skills
/plugin install ssh-remote-connection@polyakov-claude-skills
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-03 by tashan (https://tashan.sh) from public evidence. Scorer s5.
