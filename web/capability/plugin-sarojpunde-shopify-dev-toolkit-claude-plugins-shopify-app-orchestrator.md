# Shopify App Orchestrator

> Multi-agent task coordinator for full-stack Shopify app features spanning database, API, and UI layers. Coordinates complex workflows and ensures proper integration.

## Facts
- Page: https://tashan.sh/capability/plugin-sarojpunde-shopify-dev-toolkit-claude-plugins-shopify-app-orchestrator
- tashan id: plugin:sarojpunde/shopify-dev-toolkit-claude-plugins/shopify-app-orchestrator
- Source: https://github.com/sarojpunde/shopify-dev-toolkit-claude-plugins
- Type: plugin
- Category: devtools
- tashan score: 14.0 / 100
- Adoption: 7.0
- Upkeep: 33.0
- Freshness: not measured
- Evidence coverage: 62% of the inputs this score can use
- Health: not measured
- Instruction depth: not yet graded
- Official: no

## Install

```sh
/plugin marketplace add sarojpunde/shopify-dev-toolkit-claude-plugins
/plugin install shopify-app-orchestrator@shopify-development-toolkit
```

## Security audit
Not scanned. We audit npm-published capabilities; this one has no npm package we can resolve, or has not reached the queue. This is not a clean bill of health.

---
Measured 2026-08-04 by tashan (https://tashan.sh) from public evidence. Scorer s5.
