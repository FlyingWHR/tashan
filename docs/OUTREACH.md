...
|---|---|---|
| npm packages scanned for advisories against the version you'd install today | 7,863 | `SELECT count(*) FROM capabilities WHERE sec_scanned_at IS NOT NULL` |
| …of those, **no build provenance** — nothing proves the publisher built it | **74%** (5,819) | `SELECT sec_provenance, count(*) FROM capabilities WHERE sec_scanned_at IS NOT NULL GROUP BY 1` |
...
| Scored capabilities whose maintainer has **stopped** (archived / declared / dormant) | 757 | `WHERE tashan_score IS NOT NULL AND vitality='abandoned'` |
