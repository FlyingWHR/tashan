...
|---|---|---|
| npm packages scanned for advisories against the version you'd install today | 8,044 | `SELECT count(*) FROM capabilities WHERE sec_scanned_at IS NOT NULL` |
| …of those, **no build provenance** — nothing proves the publisher built it | **74%** (5,949) | `SELECT sec_provenance, count(*) FROM capabilities WHERE sec_scanned_at IS NOT NULL GROUP BY 1` |
...
| Scored capabilities whose maintainer has **stopped** (archived / declared / dormant) | 901 | `WHERE tashan_score IS NOT NULL AND vitality='abandoned'` |
| Library/SDK packages found so far that can't be launched at all | 799 | `WHERE npm_runnable=0` |
