...
| …of those, **no build provenance** — nothing proves the publisher built it | **74%** (6,075) | `SELECT sec_provenance, count(*) FROM capabilities WHERE sec_scanned_at IS NOT NULL GROUP BY 1` |
...
| Scored capabilities whose maintainer has **stopped** (archived / declared / dormant) | 1,053 | `WHERE tashan_score IS NOT NULL AND vitality='abandoned'` |
