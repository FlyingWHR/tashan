...
## ✅ Terminal experience — `npx tashan` (context7-grade, zero backend)

A single-file, dependency-free CLI that reads the **live static `index.json`** — no backend needed.
```
npx tashan search database     npx tashan top browser
npx tashan info context7       npx tashan add context7      ← the money shot
```
