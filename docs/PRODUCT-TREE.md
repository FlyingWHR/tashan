...
|---|---|---|---|---|---|
| `/index.html` | The Index. Find a capability by job or category, ranked and audited. | hero, index, ridge, site, terminal | board.json, capabilities.json, categories.json, index.json, tags.json, tasks.json | free | 119 |
| `/pricing.html` | What Pro costs and exactly what it adds. | site, terminal | board.json, index.json, tasks.json | sells | 73 |
| `/audit.html` | Paste your MCP config and see every risk we hold about what is in it. Parsed in the browser, so keys never leave the page. | audit, site, terminal | board.json, index.json, tasks.json | free | 71 |
...
| `/browse.html` | Parent index for every category and task hub — the full taxonomy. | — | — | free | 63 |
| `/methodology.html` | How every number is derived, so the score is re-checkable. | methodology, site, terminal | board.json, coverage.json, index.json, tasks.json | free | 45 |
| `/paid.html` | Settled x402 receipts on Base, joined to the catalog — the only signal here that is not a proxy for demand. Read through The Graph's Subgraph MCP, and the page prints the query so a reader can re-run it. | site | — | free | 37 |
| `/changes.html` | What moved in the ecosystem, dated — the only page built from a series that cannot be backfilled, and the freshest thing we publish. | site | — | free | 34 |
...
| `/terms.html` | Terms of service. | site, terminal | board.json, index.json, tasks.json | free | 31 |
...
|---|---|---|
| `/capability/*.html` | 12,662 | `pipeline/prerender.py` |
| `/category/*.html` | 113 | `pipeline/gen_hubs.py` |
